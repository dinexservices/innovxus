      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {UPCOMING_EVENTS.map((event) => (
            <div key={event.id} className="group flex flex-col bg-white rounded-[2rem] overflow-hidden shadow-xl transition-all duration-300 hover:-translate-y-2 border border-gray-100 hover:border-red-200 hover:shadow-2xl">
              {/* Card Image Wrapper */}
              <div className="relative h-64 overflow-hidden">
                ...
                {/* Floating Date Badge (Light Mode) */}
                <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-md px-4 py-2 rounded-2xl flex flex-col items-center justify-center shadow-lg border border-white/50">
                  <span className="text-red-600 font-bold text-xl leading-none">{event.day}</span>
                  <span className="text-black text-[10px] font-bold uppercase tracking-tighter mt-1">{event.month}</span>
                </div>

                {/* Floating Bookmark/Category Icon (Light Mode) */}
                <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-md p-2.5 rounded-xl shadow-lg border border-white/50 text-black transition-all hover:scale-110 cursor-pointer hover:bg-black hover:text-white">
              ...
              <div className="p-8 flex flex-col flex-1">
                <h4 className="text-2xl font-bold text-black mb-4 group-hover:text-red-600 transition-colors">
                  {event.title}
                </h4>
                ...
                <button className="w-full py-4 bg-gray-50 text-black rounded-2xl font-bold border border-gray-100 hover:bg-black hover:text-white transition-all duration-300 transform active:scale-95">
                  Get Tickets
                </button>
              </div>
            </div>
