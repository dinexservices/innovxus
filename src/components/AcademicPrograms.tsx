...
import { Terminal, Cpu, Shield, Lightbulb, Presentation, Code2, Database, PenTool, GitBranch, Trophy, Rocket, GraduationCap } from 'lucide-react';
                    ...
                    <div className="lg:w-2/3">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {programs.map((prog, idx) => (
                                <div key={idx} className="flex items-center gap-4 p-4 rounded-xl bg-gray-50 border border-gray-100 hover:border-red-200 hover:shadow-md transition-all duration-300 group">
                                    <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-red-600 shadow-sm group-hover:bg-red-600 group-hover:text-white transition-colors">
                                        {prog.icon}
                                    </div>
                                    <span className="font-semibold text-gray-800 text-sm md:text-base">{prog.name}</span>
                                </div>
                            ))}
                        </div>

                        <div className="mt-8 p-4 bg-red-50 text-red-800 text-sm font-medium rounded-lg inline-block">
